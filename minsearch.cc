...
#include "/rhome/zhihui/anaconda3/include/gflags/gflags.h"
...
void UpdateEdgeTriangles(Graph *__restrict__ g,
  ...
  edge_t inters = 0;
  /*
  if  ( (s->adj_list_end[i]-g->adj_start[i]+s->adj_list_end[j]-g->adj_start[j]) < SmallNumEdges ) {
  ...
  */
  {
      MinSearchSmaller(
      g, s, g->adj_start[i], s->adj_list_end[i], g->adj_start[j],
      s->adj_list_end[j], [&](edge_t k, edge_t l) {
        edge_t edg2 = s->edge_id[k];
        edge_t edg3 = s->edge_id[l];
        bool edge2_deleted = s->edge_deleted[edg2 >> 5] & (1U << (edg2 & 0x1F));
...
#pragma omp parallel for schedule(guided)
  ...
  /*
    ...
    for (edge_t cnt = s->adj_list_fwd_start[i]; cnt < s->adj_list_end[i];
      ...
      thread_local std::vector<node_t> jlocal_counts;
      jlocal_counts.clear();
      jlocal_counts.resize(s->adj_list_end[j] - s->adj_list_fwd_start[j]);
      if (s->adj_list_end[j] - s->adj_list_fwd_start[j] > s->adj_list_end[i] -cnt -1 ) {
          SMinSearchSmaller(g,s, cnt + 1, s->adj_list_end[i], s->adj_list_fwd_start[j],
                     s->adj_list_end[j], [&](edge_t k, edge_t l) {
                       edge_t edg3 = s->edge_id[l];
                       //edge_t edg2 = s->edge_id[k];
                       local_counts[cnt - s->adj_list_fwd_start[i]]++;
                       ...
                       s->edge_triangles_count[edg3].fetch_add(
                           1, std::memory_order_relaxed);
                       //s->edge_triangles_count[edg2].fetch_add(
                       //    1, std::memory_order_relaxed);
                       return true;
                     },i,j);
      } else {
          SMinSearchSmaller(g,s, s->adj_list_fwd_start[j], s->adj_list_end[j], 
                                 cnt + 1, s->adj_list_end[i], 
                                 [&](edge_t k, edge_t l) {
                                 edge_t edg3 = s->edge_id[l];
                                 local_counts[cnt - s->adj_list_fwd_start[i]]++;
                                 jlocal_counts[k - s->adj_list_fwd_start[j]]++;
                                 s->edge_triangles_count[edg3].fetch_add(
                           1, std::memory_order_relaxed);
                       return true;
                     },j,i);
          for (edge_t cnt = s->adj_list_fwd_start[j]; cnt < s->adj_list_end[j];
               cnt++) {
                   s->edge_triangles_count[s->edge_id[cnt]].fetch_add(
                   local_counts[cnt - s->adj_list_fwd_start[j]],
                   std::memory_order_relaxed);
          }
      }
    }
